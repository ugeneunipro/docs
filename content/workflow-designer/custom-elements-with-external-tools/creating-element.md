...
On the next wizard page, you can optionally input the description of the element. It would be shown on the element on the [_Scene_](/workflow-designer/introduction/workflow-designer-window-components/). The description can be parameterized. This means that if you input, for example, an attribute name (with prefix $), the name on the element would be substituted with the value of the corresponding parameter. For example, input the following parameters:
